...
Important
---------
The arXiv Atom API returns structured metadata and abstracts. It does not
...
that the available passage source is the abstract.
"""
...
def search(
    ...
    *,
    ...
    """
    Search arXiv and return structured research evidence.
    ...
    sort_by:
        arXiv sorting mode. Usually "relevance", "submitted", or "updated".

    sort_order:
        "ascending" or "descending".

    include_passages:
        Whether to construct evidence passages from the abstract.
    ...
    Notes
    -----
    The function is fault tolerant. A failed external research source
    returns an empty list instead of crashing the entire pipeline.
    """
...
def search_pages(
    ...
    *,
    ...
    Useful when the Research Agent wants broader evidence discovery.
    """
...
def _normalize_query(query: Any) -> str:
    """
    Normalize researcher-provided search text.
    """
...
def _normalize_sort_by(sort_by: Any) -> str:
    ...
    return allowed.get(value, "relevance")


def _normalize_sort_order(sort_order: Any) -> str:
    """
    Normalize arXiv sortOrder values.
    """

    value = str(sort_order or "descending").strip().lower()
...
def _build_search_expression(query: str) -> str:
    ...
    Natural-language questions are transformed into title/abstract
    concept searches.

    Existing arXiv field expressions are preserved.
    """
...
def _parse_entry(
    ...
    *,
    ...
    """
    Convert one Atom entry into a structured evidence record.
    """
    ...
    arxiv_url = _element_text(entry, "atom:id")

    authors: List[str] = []

    for author in entry.findall("atom:author", ATOM_NS):
        name = _element_text(author, "atom:name")
    ...
    identifier = _extract_arxiv_id(arxiv_url)

    if "pdf" not in links and identifier:
        links["pdf"] = (
            f"https://arxiv.org/pdf/{quote_plus(identifier)}"
        )
    ...
    record: Dict[str, Any] = {
        ...
        "evidence": {
            ...
            "quantitative_claims": _extract_quantitative_claims(
                summary
            ),
...
def _extract_evidence_passages(
    ...
    The passages are explicitly labelled as abstract passages.
    """
...
def _extract_sentences(
    ...
    """
    Extract sentences containing any requested keywords.
    """
...
def _split_sentences(text: str) -> List[str]:
    """
    Basic sentence segmentation suitable for abstract processing.
    """
...
def _extract_arxiv_id(url: str) -> str:
    """
    Extract an arXiv identifier from an abstract or PDF URL.
    """
...
def _deduplicate_papers(
    ...
    """
    Remove duplicate papers using arXiv ID, URL, or title.
    """
